...
TEST_P(D3Test, HandleOperations) {
    d3_handle_t *handle = init_d3_handle(elements, max_length, max_length, cutoff_radius, coordination_number_cutoff,
...
TEST_P(D3Test, EnergyNegative) {
    compute_dispersion_energy((real_t(*)[3])atoms, elements, max_length, cell, cutoff_radius,
                              coordination_number_cutoff, damping_type, functional_type, &energy, force, stress);
...
TEST_P(D3Test, ResultStable) {
    ...
    for (int run = 0; run < num_runs; ++run) {
        ...
        compute_dispersion_energy((real_t(*)[3])atoms, elements, max_length, cell, cutoff_radius,
                                  coordination_number_cutoff, damping_type, functional_type, &current_energy,
                                  current_force, current_stress);

        // Check stability of results
        EXPECT_NEAR(current_energy, energy, energy_tolerance) << "Energy not stable between initial and run " << run;
...
TEST_P(D3Test, NumericForceMatch) {
    ...
    real_t tolerance =
        1e-3f; // Tolerance, relatively large because numerical differentiation can bring significant errors
    ...
    for (size_t atom_idx = 0; atom_idx < max_length; ++atom_idx) {
        for (size_t component = 0; component < 3; ++component) {
            ...
            real_t *dummy_force =
                (real_t *)malloc(max_length * 3 * sizeof(real_t)); // Temporary force for numerical differentiation
            ...
            compute_dispersion_energy((real_t(*)[3])tmp_atoms, elements, max_length, cell, cutoff_radius,
                                      coordination_number_cutoff, damping_type, functional_type, &energy_plus,
                                      dummy_force, dummy_stress);
            ...
            compute_dispersion_energy((real_t(*)[3])tmp_atoms, elements, max_length, cell, cutoff_radius,
                                      coordination_number_cutoff, damping_type, functional_type, &energy_minus,
                                      dummy_force, dummy_stress);
            ...
            EXPECT_NEAR(numerical_force, force[atom_idx * 3 + component], tolerance)
                << "Force mismatch for atom " << atom_idx << ", component " << component << ": expected "
                << force[atom_idx * 3 + component] << ", got " << numerical_force;
...
TEST_P(D3Test, SupercellConsistency) {
    ...
    compute_dispersion_energy((real_t(*)[3])atoms, elements, max_length, cell, cutoff_radius,
                              coordination_number_cutoff, damping_type, functional_type, &original_energy,
                              original_force, original_stress);
    ...
    // Initialize supercell arrays to zero to avoid uninitialized memory
    memset(supercell_elements, 0, supercell_length * sizeof(uint16_t));
    memset(supercell_atoms, 0, supercell_length * 3 * sizeof(real_t));
    memset(supercell_force, 0, supercell_length * 3 * sizeof(real_t));
        ...
        for (int y = 0; y < supercell_dims[1]; ++y) {
            for (int z = 0; z < supercell_dims[2]; ++z) {
                for (size_t i = 0; i < max_length; ++i) {
                    ...
                    supercell_atoms[atom_index * 3 + 0] =
                        atoms[i * 3 + 0] + x * cell[0][0] + y * cell[1][0] + z * cell[2][0];
                    supercell_atoms[atom_index * 3 + 1] =
                        atoms[i * 3 + 1] + x * cell[0][1] + y * cell[1][1] + z * cell[2][1];
                    supercell_atoms[atom_index * 3 + 2] =
                        atoms[i * 3 + 2] + x * cell[0][2] + y * cell[1][2] + z * cell[2][2];
                    atom_index++;
                }
            }
        }
    ...
    compute_dispersion_energy((real_t(*)[3])supercell_atoms, supercell_elements, supercell_length, supercell_cell,
                              cutoff_radius, coordination_number_cutoff, damping_type, functional_type,
                              &supercell_energy, supercell_force, supercell_stress);
    ...
    for (int x = 0; x < supercell_dims[0]; ++x) {
        for (int y = 0; y < supercell_dims[1]; ++y) {
            for (int z = 0; z < supercell_dims[2]; ++z) {
                if (x == 0 && y == 0 && z == 0)
                    continue; // Skip first unit cell (reference)

                for (size_t i = 0; i < max_length; ++i) {
                    size_t ref_idx = i;
                    size_t test_idx =
                        i + (x * supercell_dims[1] * supercell_dims[2] + y * supercell_dims[2] + z) * max_length;

                    for (size_t j = 0; j < 3; ++j) {
                        EXPECT_NEAR(supercell_force[ref_idx * 3 + j], supercell_force[test_idx * 3 + j],
                                    force_tolerance)
                            << "Force mismatch on equivalent atoms: ref atom " << ref_idx << ", test atom " << test_idx
                            << ", component " << j;
                    }
                }
            }
    ...
    for (size_t i = 0; i < 9; ++i) {
        EXPECT_NEAR(supercell_stress[i], original_stress[i], stress_tolerance)
...
TestConfig generate_crystal() {
    ...
    // Cell dimensions
    real_t cell[3][3] = {
        {nx * lattice_constant, 0.0f, 0.0f}, {0.0f, ny * lattice_constant, 0.0f}, {0.0f, 0.0f, nz * lattice_constant}};
    ...
    for (int i = 0; i < nx; i++) {
        for (int j = 0; j < ny; j++) {
            for (int k = 0; k < nz; k++) {
                // Corner atom
                atoms.push_back({i * lattice_constant, j * lattice_constant, k * lattice_constant});

                // Body-centered atom
                atoms.push_back(
                    {(i + 0.5f) * lattice_constant, (j + 0.5f) * lattice_constant, (k + 0.5f) * lattice_constant});
            }
        }
    }

    return {"BCC_Fe_Crystal",
            elements,
...
TestConfig small_system = {
    ...
    {{5.1372f, 5.5512f, 10.1047f},
     ...
     {4.7678f, 7.2075f, 11.4098f}}, // atoms
    {{20.0f, 0.0f, 0.0f}, {0.0f, 20.0f, 0.0f}, {0.0f, 0.0f, 20.0f}} // cell
};
...
INSTANTIATE_TEST_SUITE_P(SmokeTest, D3Test,
                         ::testing::Values(small_system, crystal_system, surface_system, small_system_BJ,
                                           crystal_system_BJ, surface_system_BJ),
