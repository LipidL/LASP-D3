...
void set_atoms(D3Handle_t *handle, real_t *coords, uint16_t *elements, uint64_t length) {
    ...
    for (uint64_t i = 0; i < length; ++i) {
        bohr_coords[i][0] = coords[i * 3] * UNITS::ANGSTROM_TO_BOHR;
        bohr_coords[i][1] = coords[i * 3 + 1] * UNITS::ANGSTROM_TO_BOHR;
        bohr_coords[i][2] = coords[i * 3 + 2] * UNITS::ANGSTROM_TO_BOHR;
    }
    try {
...
void set_cell(D3Handle_t *handle, real_t cell[3][3]) {
    ...
    for (int i = 0; i < 3; ++i) {
        for (int j = 0; j < 3; ++j) {
            bohr_cell[i][j] = cell[i][j] * UNITS::ANGSTROM_TO_BOHR;
        }
    }
    try {
...
int main() {
    ...
    real_t atoms[10][3] = {{5.1372f, 5.5512f, 10.1047f}, {4.5169f, 6.1365f, 11.3604f}, {6.1937f, 4.4752f, 10.2703f},
                           {4.7872f, 5.9358f, 8.9937f},  {6.7474f, 4.3475f, 9.3339f},  {5.6975f, 3.5214f, 10.5181f},
                           {6.8870f, 4.7006f, 11.0939f}, {4.8579f, 5.6442f, 12.2774f}, {3.4204f, 6.0677f, 11.2935f},
    ...
    real_t cell[3][3] = {{200.0f, 0.0f, 0.0f}, {0.0f, 20.0f, 0.0f}, {0.0f, 0.0f, 20.0f}};
    for (uint64_t i = 0; i < 3; ++i) {
        for (uint64_t j = 0; j < 3; ++j) {
    ...
    compute_dispersion_energy(atoms, elements, 10, cell, cutoff_radius, CN_cutoff_radius, ZERO_DAMPING, PBE0, &energy,
                              force, stress);
    debug("energy: %f eV\n", energy);
