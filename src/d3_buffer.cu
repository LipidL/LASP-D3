...
uint16_t Unique_Elements::find(uint16_t element) {
    ...
    exit(EXIT_FAILURE);
} // find the index of the element in the unique elements array, if not found, it will add the element to the array and return the index
...
__host__ Device_Buffer::Device_Buffer(real_t coords[][3], uint16_t *elements, real_t cell[3][3], uint64_t length, real_t cutoff, real_t CN_cutoff, uint64_t max_neighbors){
