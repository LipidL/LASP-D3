...
__host__ Device_Buffer::Device_Buffer(real_t coords[][3], uint16_t *elements, uint64_t length_elements,
    ...
    }
    {
        ...
        for (uint16_t i = 0; i < 3; ++i) {
            // calculate the norm of reciprocal lattice vector, note that in host_data_.cell, cell vectors are stored in
            // rows
                ...
                for (uint16_t k = 0; k < NUM_REF_C6; ++k) {
                    for (uint16_t l = 0; l < NUM_REF_C6; ++l) {
                        ...
                        for (uint16_t m = 0; m < NUM_C6AB_ENTRIES; ++m) {
                            h_c6ab_ref[index + m] = c6ab_ref[element_i - 1][element_j - 1][k][l][m];
                        }
                    }
                }
            }
...
__host__ void Device_Buffer::set_atoms(uint16_t *elements, real_t coords[][3], uint64_t length) {
    ...
    for (uint64_t i = 0; i < length; ++i) {
        ...
        unique_elements.find(
            elements[i]); // check that the element is in the unique elements array. if not found, it will crash.
    }
...
__host__ void Device_Buffer::construct_grids() {
    ...
    for (uint64_t i = 0; i < num_atoms; ++i) {
        ...
        for (uint8_t j = 0; j < 3; ++j) {
             ...
             * (increment the supercell_idx to avoid atom in the same grid but actually have different supercell indices
             * being wrapped to different grids)
             */
    ...
    // copy data to device
    CHECK_CUDA(
        cudaMemcpy(this->host_data_.atom_types, h_atom_types, num_atoms * sizeof(uint64_t), cudaMemcpyHostToDevice));
