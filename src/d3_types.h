...
typedef struct device_data {
    ...
    real_t*
        c6_ab_ref;  // size:
                    // num_elements*num_elements*NUM_REF_C6*NUM_REF_C6*NUM_C6AB_ENTRIES
    uint64_t c6_stride_1, c6_stride_2, c6_stride_3,
        c6_stride_4;    // strides for c6ab array
    ...
    uint64_t
        max_cell_bias[3];  // the maximum bias of the cell in each direction,
    ...
    real_t* coordination_numbers;  // array of coordination numbers, length:
                                   // num_atoms.
    uint16_t status;  // status of the calculation process, 0:normal, 0b01:
                      // neighbor list overflow detected
